...
@section('content')
            ...
            <div class="container">
                ...
                <div class="bg-white">
                    @foreach ($featured_categories as $key => $category)
                        <a class="hov-bg-primary hov-text-white fw-700 fs-14 d-inline-block" href="{{ route('products.category', $category->slug) }}" title="{{  $category->getTranslation('name')  }}" style="margin: 2px 4px 2px 0;background: rgb(255, 239, 207);; font-size: 13px;border: 1px solid #ccc;color:#777;border-radius: 3px;padding: 0.3846em;line-height: 1.2;">{{ $category->getTranslation('name') }}</a>
                    @endforeach
                </div>
            </div>
        </section>   
    ...
    <!-- Flash Deal -->
    @php
        $flash_deals = \App\Models\FlashDeal::where('status', 1)->orderByDesc('featured')->get();
    @endphp
        ...
        <section class="mb-2 mb-md-3 mt-2 mt-md-3">
            <div class="container" style="background-color: {{$flash_deal->background_color ?? ($flash_deal->featured ? '#ffefcf' : '')}}">
                ...
                <div class="d-flex flex-wrap mb-2 mb-md-3 align-items-baseline justify-content-between">
                    ...
                    <h3 class="fs-16 fs-md-20 fw-700 mb-2 mb-sm-0">
                        @if ($flash_deal->featured == 1)
                            <img style="width: 100px;margin-left:6px" src="{{ static_asset('assets/img/hot-deal-logo.gif') }}" title="Hot Deal">
                        @else
                            <span class="d-inline-block">{{ translate($flash_deal->getTranslation('title')) }}</span>
                ...
                <div class="bg-white mb-3 d-md-none">
                    <div class="aiz-count-down-circle p-1" end-date="{{ date('Y/m/d H:i:s', $flash_deal->end_date) }}"></div>
                </div>
                ...
                <div class="row gutters-5 gutters-md-16">
                    ...
                    <div class="col">
                        ...
                        @endphp
                        <div class="aiz-carousel border-top @if(count($flash_deals)>8) border-right @endif arrow-inactive-none arrow-x-0"data-items="6" data-xxl-items="6" data-xl-items="4" data-lg-items="4" data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows="true" data-dots="false" data-autoplay="true" data-infinite="true">
                            @php
                                $col = ceil(count($flash_deals)/2);
                                $init = 0 ; $end = 1 ;
                            @endphp
                            @for ($i = 0; $i < $col; $i++)
                                ...
                                @foreach ($flash_deals as $key => $flash_deal_product)
                                    @if ($key >= $init && $key <= $end)
                                        ...
                                        @if ($product != null && $product->published != 0)
                                            ...
                                            @endphp
                                            <div class="h-lg-auto flash-deal-item position-relative text-center border-bottom @if($i!=4) border-right @endif has-transition hov-shadow-out hov-animate-outline z-1 p-1">
                                                @include('frontend.partials.product_box_1', compact('product', 'product_url'))
                                            </div>
                                        @endif
                                    @endif
                                @endforeach
                                    ...
                                    @foreach ($todays_deal_products as $key => $product)
                                        <div class="carousel-box h-100 px-3 px-lg-0">
                                            <a href="{{ route('product', $product->slug) }}" class="h-100 overflow-hidden hov-scale-img mx-auto" title="{{  $product->getTranslation('name')  }}">
                                                <!-- Image -->
                                                <div class="img h-80px w-80px rounded-sm overflow-hidden mx-auto">
                ...
                <div class="container">
                    ...
                    <div class="px-sm-3">
                        <div class="aiz-carousel arrow-none sm-gutters-16" data-items="6" data-xl-items="5" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true' data-infinite='false'>
                            ...
                            <div class="carousel-box p-1 position-relative has-transition border-right border-top border-bottom @if($key == 0) border-left @endif hov-animate-outline">
